...
?[end]?
@[end]@
function validateSearchFilter returns log (buffer bf#[app.component,AblTempFilter,PascalCase]#Filter for tmp#[app.component,AblTempFilter,PascalCase]#Filter):
...
@[end]@
    define output       parameter table for tmp#[app.component,AblTempTable,PascalCase]#.
    define input-output parameter table for rowErrors.

    empty temp-table tmp#[app.component,AblTempTable,PascalCase]#.
    for first #[app.table]#
@[app.fields,isPrimary]@
...
procedure getByFilter:
    ...
    define input        parameter table       for tmp#[app.component,AblTempFilter,PascalCase]#Filter.
    ...
    run adjustSearchFields(input-output table tmp#[app.component,AblTempFilter,PascalCase]#Filter).

    find first tmp#[app.component,AblTempFilter,PascalCase]#Filter no-lock no-error.
    if (not avail tmp#[app.component,AblTempFilter,PascalCase]#Filter)
    or (not validateSearchFilter(buffer tmp#[app.component,AblTempFilter,PascalCase]#Filter))
    then do:
    ...
    for first tmp#[app.component,AblTempFilter,PascalCase]#Filter:
        oWhere = oQuery:and().

@[app.fields,isFilter&!isRangeFilter]@
        if  (tmp#[app.component,AblTempFilter,PascalCase]#Filter.#[field]# <> ?)
        and (tmp#[app.component,AblTempFilter,PascalCase]#Filter.#[field]# ?[ablType=character]?<> ""?[end]??[ablType=integer]?> 0?[end]??[!ablType=character&!ablType=integer]?<> ??[end]?)
        then oWhere:and("#[app.table]#.#[field]#", tmp#[app.component,AblTempFilter,PascalCase]#Filter.#[field]#?[ablType=character]?, oWhere:OPERATOR_BG?[end]?).
...
@[end]@
        if  tmp#[app.component,AblTempFilter,PascalCase]#Filter.ds-query <> ?
        and tmp#[app.component,AblTempFilter,PascalCase]#Filter.ds-query <> ""
        then do:
            assign in-search-value-aux = integer(tmp#[app.component,AblTempFilter,PascalCase]#Filter.ds-query) no-error.
            assign lg-value-integer-aux = not error-status:error.

            oWhere = oQuery:and().
@[app.fields,isFilter&ablType=character]@
            oWhere:or("#[app.table]#.#[field]#", tmp#[app.component,AblTempFilter,PascalCase]#Filter.ds-query, oWhere:OPERATOR_BG).
@[end]@
...
procedure createRecord:
    ...
    define input-output param table for tmp#[app.component,AblTempTable,PascalCase]#.
    define input-output param table for rowErrors.

    find first tmp#[app.component,AblTempTable,PascalCase]# no-error.

    if not avail tmp#[app.component,AblTempTable,PascalCase]#
    then do:
...
@[app.fields,isAuto]@
?[isFirst]?
    assign tmp#[app.component,AblTempTable,PascalCase]#.#[field]# = nextSequence().
?[end]?
@[end]@
    if can-find(first #[app.table]#
@[app.fields,isPrimary]@
                ?[isFirst]?where ?[end]??[!isFirst]?  and ?[end]?#[app.table]#.#[field]# = tmp#[app.component,AblTempTable,PascalCase]#.#[field]#
@[end]@)
    then do:
    ...
    run validateRecord (
        input table tmp#[app.component,AblTempTable,PascalCase]#,
        input-output table rowErrors).
...
procedure updateRecord:
    ...
    if not available tmp#[app.component,AblTempTable,PascalCase]#
    then do:
        run insertOtherError(
            input 0,
            input "Nao foram encontrados dados para a insercao/atualizacao do registro",
...
procedure assignRecord private:
    ...
    buffer-copy tmp#[app.component,AblTempTable,PascalCase]#
...
procedure adjustSearchFields private:
    def input-output param table for tmp#[app.component,AblTempFilter,PascalCase]#Filter.
